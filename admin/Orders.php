...
$selectAllOrdersSql = "SELECT orders.* , users.name FROM orders , users WHERE orders.user_id = users.id";
$selectAllOrdersResult = runQuery($selectAllOrdersSql);

$select_order_detais_Sql = "SELECT * FROM `order_details`";
$selectOrderDetailsResult = runQuery($select_order_detais_Sql);
            ...
            if ($selectAllOrdersResult->num_rows > 0) {
                while ($row = $selectAllOrdersResult->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="success edit-btn" style="color:white;width: 80px;height:30px;background-color: #AFE1AF;-webkit-transition: .3s ease-out;transition: .3s ease-out;border-radius: 8px;border: none;" type="button"
