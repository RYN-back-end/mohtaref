...
if (isset($_POST['email']) && isset($_POST['password'])) {
    $sql = "SELECT * FROM `professionals` WHERE `email` =  '{$_POST['email']}'";
    $data = runQuery($sql);
    if ($data->num_rows > 0) {
        $row = $data->fetch_assoc();
        if (password_verify($_POST['password'], $row['password'])) {
			if ($row['active'] == 0) {
                        header('Location: login.php?error=في انتظار تنشيط حسابك من مدير النظام');
                        die();
                    } elseif ($row['active'] == 1) {
                        
            $_SESSION['professional']['id'] = $row['id'];
            $_SESSION['professional']['title'] = $row['title'];
            $_SESSION['professional']['banner'] = $row['banner'];
            $_SESSION['professional']['email'] = $row['email'];
            $_SESSION['professional']['desc'] = $row['desc'];
            $_SESSION['professional']['whatsapp'] = $row['whatsapp'];
            $_SESSION['professional']['images'] = $row['images'];
            $_SESSION['professional']['loggedin'] = true;
            ...
            <div class="col-8-md">
                ...
                <form action="" method="post"><p>تسجيل الدخول</p>
                    ...
                    <div class="form-control"><label for="password">كلمة المرور </label><input type="password" required
                                                                                               name="password"></div>
                    <div class="details-login"><p> ليس لدي حساب؟<a href="craftregister.php">اضغط هنا</a></p></div>
