...
checkLogin();
// Fetch carts details
$selectCartSql = "SELECT cart.qty, products.price AS product_price, SUM(products.price * cart.qty) AS total_price
 FROM cart 
                  INNER JOIN products ON cart.product_id = products.id
                  WHERE cart.user_id = '{$_SESSION['user']['id']}'";

$selectCartsResult = runQuery($selectCartSql)->fetch_assoc();


if (isset($_POST['name'])) {
    $insertSql = "INSERT INTO `payments` (`pay_name`,`pay_no`,`valid_date`,`cvv`,`user_id`) VALUES ('{$_POST['name']}','{$_POST['number']}','{$_POST['valid_date']}','{$_POST['cvv']}','{$_SESSION['user']['id']}')";
    runQuery($insertSql);

    $insertMain = "INSERT INTO `orders` (`user_id`,`total_price`,`date`)VALUES ('{$_SESSION['user']['id']}','{$selectCartsResult['total_price']}',CURRENT_TIMESTAMP)";
    runQuery($insertMain);
    ...
    foreach ($allCart as $detail){
        $netTotal = $detail['qty']*$detail['product']['price'];
        $insertProducts = "INSERT INTO `order_details` (`order_id`,`product_id`,`professional_id`,`qty`,`price`,`total`) VALUES ('{$orderId}','{$detail['product_id']}','{$detail['product']['professional_id']}','{$detail['qty']}','{$detail['product']['price']}','{$netTotal}')";
    ...
    header("location: Orders.php?success=تم تأكيد الدفع");
}
    ...
    <div id="Checkout" class="inline">
        ...
        <form action="" method="post">
            <div class="form-group">
                <label for="PaymentAmount">دفع مبلغ</label>
                <div class="amount-placeholder">
                    <span>ر.س</span>
                    <span><?php echo $selectCartsResult['total_price']?>  </span>
                </div>
            </div>
            <div class="form-group">
                <label or="NameOnCard">اسم صاحب البطاقة</label>
                <input id="NameOnCard" class="form-control" required  name="name"  type="text" maxlength="255"></input>
            </div>
            <div class="form-group">
